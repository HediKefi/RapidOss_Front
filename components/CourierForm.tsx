...
export default function CourierForm({
  ...
  dict: Dictionary["courierPage"]["form"];
  villes: string[];
}) {
  ...
  const submit = (e: React.FormEvent) => {
    ...
    if (!fields.vehicle) next.vehicle = dict.errors.vehicle;
    if (!fields.availability) next.availability = dict.errors.availability;
    setErrors(next);
    if (Object.keys(next).length > 0) return;

    // demo site — simulate the fleet desk acknowledging the application
    setSending(true);
    setReference(`L-${Date.now().toString().slice(-6)}`);
    setTimeout(() => {
      setSending(false);
      setSent(true);
    }, 1600);
      ...
      {sent ? (
        ...
        >
          <div className="grid gap-7 md:grid-cols-2">
            <div>
              <label htmlFor="liv-name" className={labelCls}>
                {dict.name}
              </label>
              <input
                id="liv-name"
            ...
            <div>
              <label htmlFor="liv-phone" className={labelCls}>
                {dict.phone}
              </label>
              <div
            ...
            <div>
              <label htmlFor="liv-email" className={labelCls}>
                {dict.email}
              </label>
              <input
                id="liv-email"
            ...
            <div>
              <label htmlFor="liv-ville" className={labelCls}>
                {dict.ville}
              </label>
              <select
                id="liv-ville"
          ...
          <fieldset className="mt-8">
            <legend className={labelCls}>{dict.vehicle}</legend>
            <div className="grid grid-cols-2 gap-2 sm:grid-cols-4">
              {dict.vehicleOptions.map((v) => (
                <button
                  key={v}
                  type="button"
                  onClick={() => set("vehicle", v)}
                  aria-pressed={fields.vehicle === v}
          ...
          <fieldset className="mt-8">
            <legend className={labelCls}>{dict.availability}</legend>
            <div className="flex flex-wrap gap-2">
              {dict.availabilityOptions.map((a) => {
                const on = fields.availability === a;
                return (
                  <button
                    key={a}
                    type="button"
